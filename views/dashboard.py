...
from queries import get_insert_data as gid
...
def btn_primary(parent, text, command, width, height):
...
def main_view():
    ...
    history_btn.pack(anchor="w", padx=10, side="left")
    ...
    # Footer frame
    footer_frame = ctk.CTkFrame(app, fg_color=DARK_COLOR["surface"],
                                width=1050, height=50,
                                border_width=1,
                                border_color=DARK_COLOR["border"],
                                corner_radius=DIMENSIONS["border_radius"])
    footer_frame.pack_propagate(False)
    footer_frame.pack(pady=10, padx=10)

    updt_label = ctk.CTkLabel(footer_frame, text="Mis à jour depuis : ",
                            font=FONTS["label"],
                            text_color=DARK_COLOR["bg"]).pack()
